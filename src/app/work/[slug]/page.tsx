...
export function generateStaticParams() {
  return projects.filter((project) => project.internalPage).map((project) => ({ slug: project.slug }))
}
...
export default async function WorkDetailPage({ params }: Props) {
  ...
  const images = Array.from({ length: project.slideCount }, (_, index) => `/work-images/${project.slug}/screen${index + 1}.${extension}`)
  ...
  return (
    <>
      ...
      <article className="work-copy post-content">
        {project.content.map((paragraph) => <p key={paragraph} dangerouslySetInnerHTML={{ __html: paragraph }} />)}
      </article>
